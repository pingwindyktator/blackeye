...
from dnslib import DNSRecord, RR, QTYPE, RCODE, parse_time, dns
...
class InterceptResolver(BaseResolver):
    ...
    def __init__(self, address, port, ttl, intercept, skip, nxdomain, timeout=0):
        ...
        self.zone = []
        self.overwrite = {'facebook.com': '216.58.215.78'}
    ...
    def resolve(self, request, handler):
        ...
        for addr, ip in self.overwrite.items():
            if qname.matchGlob(addr):
                ...
                new_reply = DNSRecord(reply.header, reply.questions, [a], reply.q, None, reply.auth, None)
                reply = new_reply
